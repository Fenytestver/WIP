...
#define PIN_BUTTON_SNOOZE C2
#define PIN_BUZZER D7
#define PIN_LED_GREEN D5
#define PIN_LED_YELLOW D4
#define PIN_LED_RED D3
#define PIN_LED_RED2 D2
#define PIN_LED_RED3 C3
#define PIN_SIREN A1 // unused pin, no output
#define PIN_OUT_TECHNICAL A4 // BJT1 for Announcer
#define PIN_OUT_CRITICAL_A A3 // BJT2 for Announcer
#define PIN_OUT_CRITICAL_B A2 // BJT3 for Announcer
...
HardwarePinLed* out_technical; // to Announcer
HardwarePinLed* out_critical_a; // to Announcer
HardwarePinLed* out_critical_b; // to Announcer
RealSiren* siren;
...
void setup() {
  ...
  out_technical = new HardwarePinLed(PIN_OUT_TECHNICAL); // to Announcer
  out_critical_a = new HardwarePinLed(PIN_OUT_CRITICAL_A); // to Announcer
  out_critical_b = new HardwarePinLed(PIN_OUT_CRITICAL_B); // to Announcer
  ...
  siren->setup();
  out_technical->setup();
  out_critical_a->setup();
  out_critical_b->setup();
...
void loop() {
  ...
  if (numDevices > 0) {
    ...
    // Green light proc begins
    ledGreen->setState(!technical && !critical && armed > 0 && maintenance == 0);
    // EOF Green light proc

    // Yello light proc begins
    ...
    // Red lights proc begins
    if (maintenance == 0 && critical && isCriticalShutoff(alarmReason) && !snoozed) {  // critical B
      renderCriticalLeds(criticalDuration, true /* shutoff */, nowbit);
    } else if (maintenance == 0 && critical && !snoozed) { // critical A
      renderCriticalLeds(criticalDuration, false /* shutoff */, nowbit);
    } else { // not critical A or B
      ledRed->setState(false);
      ledRed2->setState(false);
      ledRed3->setState(false);
      out_critical_a->setState(false);
      out_critical_b->setState(false);
      sirenOn = false;
    }
    // EOF Red lights proc

  } else { // belongs to if (numDevices > 0){else blinks yellow/green}
    ledRed->setState(false);
    ledRed2->setState(false);
    ledRed3->setState(false);
    ...
    sirenOn = false;
    lastCritical = 0L;
  } // EOF numDevices handling proc

  // siren is directed to an unused pin w/ no output!
  if (sirenOn && !snoozed) {
    siren->on();
  } else {
    siren->off();
...
void renderCriticalLeds(long duration, bool shutoff, long nowBits) {
  ...
  if (shutoff) { // critical B  incl out_critical_b
    if (sec > 180) {
      ledRed->setState(true);
      ledRed2->setState(true);
      ledRed3->setState(true);
      out_critical_b->setState(nowBits % 2);
      sirenOn = true;
    } else if (sec > 120) {
      ledRed->setState(true);
      ledRed2->setState(true);
      ledRed3->setState(true);
      out_critical_b->setState(true);
      sirenOn = false;
    } else if (sec > 60) {
      ledRed->setState(true);
      ledRed2->setState(true);
  ...
  } else { // critical A  incl out_critical_a
    sec = sec % 360;
    sirenOn = false;
    out_critical_a->setState(true);
    if (sec > 180) {
      ledRed->setState(true);
      ledRed2->setState(true);
      ledRed3->setState(true);
...
} // EOF renderCriticalLeds() incl out_critical_a and out_critical_b
